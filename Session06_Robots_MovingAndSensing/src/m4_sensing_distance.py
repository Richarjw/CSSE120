...
#          -- How far did the robot REPORT that it moved, in cm?
    ...
    robot.toFullMode()
    for _ in range(0, 3):
        left = float(input('Enter the left wheel speed:'))
        right = float(input('Enter the right wheel speed:'))
        seconds = float(input('Enter the number of seconds to move at that speed:'))
        sensor1 = new_create.Sensors.distance
        distance = robot.getSensor(sensor1)
        sensor2 = new_create.Sensors.angle
        angle = robot.getSensor(sensor2)
