...
Authors: David Mutchler, Amanda Stouder, Chandan Rupakheti, Katie Dion,
         ...
         and Jack Richard.  November 2014.
...
class Balloon(object):
    ...
    def __init__(self, capacity, owner):
        ...
        self.popped = False
    def __repr__(self):
        return 'Balloon({}, {})'.format(self.capacity, self.owner)
    ...
    def blow_air_into(self, number_of_breaths):
        ...
        if self.owner.age < 12:
            for k in range(number_of_breaths):
                self.currentVolume = self.currentVolume + 0.03
            ...
            for k in range(number_of_breaths):
                self.currentVolume = self.currentVolume + 0.5
                if self.currentVolume > self.capacity:
                    self.currentVolume = 0
                    self.owner.is_happy = False
                    self.popped = True
    ...
    def let_air_out(self, seconds):
        ...
        for k in range(seconds):
            if self.currentVolume - 0.08 < 0:
                self.currentVolume = 0
            else:
                self.currentVolume = self.currentVolume - 0.08
    ...
    def fuller_balloon(self, other_balloon):
        ...
        """
        if self.currentVolume > other_balloon.currentVolume:
            return self
        elif self.currentVolume < other_balloon.currentVolume:
            return other_balloon
        else:
            if self.capacity >= other_balloon.capacity:
                return self
            else:
                return other_balloon
    ...
    def new_balloon(self):
        ...
        of this Balloon.
        """
        newBalloon = Balloon(self.capacity, self.owner.clone())
        return newBalloon
    ...
    def unpunctured_balloons(self, balloons):
        ...
        for k in range(len(balloons)):
            if balloons[k].popped == False:
                newList.append(balloons[k])
        return newList
# ----------------------------------------------------------------------
